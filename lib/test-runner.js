...
function getFilePaths(path) {
  ...
  var modulePath = Path.dirname(relativePath)
      ...
      .replace(new RegExp('^' + testsDirName), '').replace(new RegExp('^' + scriptsDirName), '');
...
function evalFile(path) {
  ...
  if (!executedTestsCache[filePaths.relativeModulePath]) {
    executedTestsCache[filePaths.relativeModulePath] = true;
    var rtn = require(filePaths.testFile);
    var reArgs = /\(\s*([^)]+?)\s*\)/;
    var description = typeof rtn === 'function' ?
      // use the first argument variable of the test module as the describe
      // statement
      reArgs.exec(rtn)[0].replace(/^\(/, '').split(/\s*,\s*/)[0] :
      filePaths.relativeModulePath;
